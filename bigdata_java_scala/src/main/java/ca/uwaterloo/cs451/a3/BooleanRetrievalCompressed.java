...
public class BooleanRetrievalCompressed extends Configured implements Tool {
  ...
  private void initialize(String argIndexPath, String collectionPath, FileSystem argFs) throws IOException {
    // index = new MapFile.Reader(new Path(indexPath + "/part-r-00000"), fs.getConf());
    indexPath = argIndexPath + "/";
    fs = argFs;
    collection = argFs.open(new Path(collectionPath));
    stack = new Stack<>();
  }
  ...
  private ArrayListWritable<PairOfInts> fetchPostings(String term) throws IOException {
    ...
    for (FileStatus status : fileStatus) {
      String fileStr = status.getPath().toString();
      if (fileStr.contains("_SUCCESS")) continue;
      ...
      int df = WritableUtils.readVInt(dataIn);
      int doc = 0;
      int tf;

      for (int i = 0; i < df; i++) {
        doc += WritableUtils.readVInt(dataIn);
        tf = WritableUtils.readVInt(dataIn);
        postings.add(new PairOfInts(doc, tf));
      }
      byteIn.reset();
      dataIn.close();
      break;
